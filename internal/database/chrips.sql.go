...
const getUserChirp = `-- name: GetUserChirp :one
SELECT id, body, user_id, created_at, updated_at FROM chirps WHERE id = $1 AND user_id = $2
`

type GetUserChirpParams struct {
	ID     uuid.UUID
	UserID uuid.UUID
}

func (q *Queries) GetUserChirp(ctx context.Context, arg GetUserChirpParams) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, getUserChirp, arg.ID, arg.UserID)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.Body,
